...
import (
	...
	apolloConfig "github.com/xuxiaofan1101/agollo/v4/env/config"
...
func New(ctx context.Context, config Config) (adapter gcfg.Adapter, err error) {
	...
	// 创建专门的 Logger 实例
	apolloLogger := g.Log("apollo")
	apolloLogger.SetStdoutPrint(true) // 是否输出控制台
	// 传入GoFrame Logger
	goFrameLogger := NewGoFrameLogger(apolloLogger)
	agollo.SetLogger(goFrameLogger)
...
func (c *Client) Get(ctx context.Context, pattern string) (value interface{}, err error) {
	c.mu.RLock()
	if dynamicData, exists := c.dynamicData[pattern]; exists {
		c.mu.RUnlock()
...
func (c *Client) SetContent(content map[string]interface{}) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Client) GetDynamicContent() map[string]interface{} {
	c.mu.RLock()
	defer c.mu.RUnlock()
	return c.dynamicData
}
